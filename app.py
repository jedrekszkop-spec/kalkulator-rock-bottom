...
# --- OFICJALNY LOGOTYP W NAGŁÓWKU (HTML/CSS) ---
st.markdown("""
...
# Podział na dwie czyste zakładki
...
with tab1:
    ...
    if typ_gazu == "Nitrox":
        st.info(f"✨ Gaz: **Nitrox {int(fo2*100)}**. Maksymalna Głębokość Operacyjna (MOD) przy $PPO_2$=1.6 wynosi: **{mod_metry:.1f} m**.")
    else:
        st.info(f"✨ Gaz: **Powietrze**. Maksymalna Głębokość Operacyjna (MOD) przy $PPO_2$=1.6 wynosi: **{mod_metry:.1f} m**.")
    ...
    # --- BAZA DANYCH NDL (Limity bezdekompresyjne) ---
    ...
    st.info(f"⏱️ **Limit bezdekompresyjny (NDL)** dla głębokości {glebokosc}m wynosi około: **{lim_ndl} minut**.")
...
# --- MATEMATYKA FIZYCZNA PROFILU (ZASADA 200 BAR) ---
p_dno = (glebokosc / 10) + 1
p_przystanek = (6 / 10) + 1
p_powierzchnia = 1.0

# Obliczenia zużycia solo (Faza denna)
...
# Obliczenia rezerwy awaryjnej (Rock Bottom)
...
# ==========================================
# 🔘 KONSOLA STERUJĄCA DWUKAFELKOWA (Wersja 12)
# ==========================================
st.write("---")
...
with res_col2:
    if gaz_pozostaly_na_powrot_bar > 0:
        st.metric(label="📉 MANOMETR PO FAZIE DENNEJ", value=f"{gaz_pozostaly_na_powrot_bar} BAR", delta=f"Zużyłeś: {zuzycie_fazy_dennej_bar} bar")
    else:
        st.metric(label="📉 MANOMETR PO FAZIE DENNEJ", value="0 BAR", delta="Gaz wyczerpany!")

# Zintegrowane, łagodne powiadomienia taktyczne (MOD, NDL i Gaz)
if glebokosc > mod_metry:
    st.warning(f"⚠️ **OSTRZEŻENIE O ZAGROŻENIU (MOD):** Planowana głębokość ({glebokosc}m) przekracza limit operacyjny MOD ({mod_metry:.1f} m) dla tej mieszanki przy $PPO_2=1.6$!")
elif czas_na_dnie > lim_ndl:
    st.warning(f"⚠️ **OSTRZEŻENIE O DEKOMPRESJI:** Twój czas ({czas_na_dnie} min) przekracza limit bezdekompresyjny NDL ({lim_ndl} min) dla głębokości {glebokosc}m. Ryzykujesz wejście w dekompresję!")
elif gaz_pozostaly_na_powrot_bar < rock_bottom_bar:
    st.warning(f"⚠️ **PLAN PODWYŻSZONEGO RYZYKA GAZOWEGO:** Po {czas_na_dnie} min na dnie zostanie Ci {gaz_pozostaly_na_powrot_bar} bar. To MNIEJ niż wymagany Rock Bottom ({rock_bottom_bar} bar)!")
else:
    st.success(f"👉 **Wytyczne:** Startujesz z 200 bar. Po fazie dennej w butli zostanie **{gaz_pozostaly_na_powrot_bar} bar**. Ponieważ to więcej niż Twój Rock Bottom ({rock_bottom_bar} bar), nurkowanie jest bezpieczne. Wracasz na powierzchnię przy {rock_bottom_bar} bar.")

if gestosc_na_dnie > 5.2 and glebokosc <= mod_metry:
    st.warning(f"⚠️ **Uwaga:** Gęstość gazu na dnie wynosi {gestosc_na_dnie:.1f} g/l. Opór oddechowy automatu będzie nieco większy.")

# --- ANATOMIA PROCESÓW NA SAMYM DOLE ---
st.write(" ")
